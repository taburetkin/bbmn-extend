...
var ctors = [Backbone$1.Model, Backbone$1.Collection, Backbone$1.View, Backbone$1.Router, MnObject, Mn__default.Application, Mn__default.Region];
...
function triggerMethodOn(context) {
	...
	return Mn.triggerMethod.apply(context, args);
}
...
function isViewClass(arg) {
	return _.isFunction(arg) && (arg == Backbone$1.View || isView(arg.prototype));
}
...
function setInputAttributes(inputView) {
	...
	_(restrictionKeys).each(function (key2, key) {
		var value = getOption(inputView, opts, key);
...
var nativeAjax = Backbone$1__default.ajax;
...
var common = {
	_ensureOptions: function _ensureOptions() {
		...
		if (!this.$container.length) throw new Error('children container is not in the DOM');
	},
...
var customs = {
	...
	_extractCustomView: function _extractCustomView(arg) {
	...
	_injectCustoms: function _injectCustoms(items) {
		var _this = this;
		...
		_.each(customs, function (custom) {
			var view = _this._extractCustomView(custom.view, custom);
			if (!view) return;
...
var models = {
	process: function process() {
		...
		var attach = this._injectCustoms(data.attach);
	...
	_getItems: function _getItems() {
		...
		if (this._store.isFiltered) return this._store.filtered;else return this._store.items;
	},
	_filterItems: function _filterItems(items) {
		var _ref2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
		    ...
		    force = _ref2.force,
		    _ref2$initializeViews = _ref2.initializeViews;
		...
		var shouldUpdateIndex = items == this.collection.models;
...
var render = {
	...
	render: function render(data) {
		if (!data) {
			...
			if (this.view._isRendered) this.removeCustomViews();
	...
	_destroyChildViews: function _destroyChildViews() {
		...
		_.each(views, function (view) {
			_this2._destroyChildView(view, shouldDisableEvents);
		});
	...
	_detachChildViews: function _detachChildViews() {
		...
		_.each(contexts, function (context) {
			_this3._detachChildView(context.view, monitorViewEvents);
		});
	},
	...
	_attachChildViews: function _attachChildViews() {
		...
		_.each(contexts, function (context) {
			var view = _this4._ensureContextHasView(context);

			if (!view) return;

			renderView(view);
			_this4.view.Dom.appendContents(elBuffer, view.el, { _$contents: view.$el });
	...
	_ensureContextHasView: function _ensureContextHasView(context) {
		if (context.view && !context.view._isDestroyed) return context.view;else if (context.isCollection) {
...
var MergeOptions = ['createView', 'dataFilter', 'dataComparator', 'enableCollection', '$container', 'view', 'modelView', 'modelViewOptions'];
...
_.extend(ViewManager.prototype, Backbone$1.Events, borrow, collection, common, customs, models, render);



var index$13 = Object.freeze({
	historyWatcher: index$11,
	navigator: index$12,
	history: index$10,
