...
function betterResult(obj, key) {
	...
	var value = obj[key];
	if (force === false || !_.isFunction(value) || isKnownCtor(value)) return value;
	...
	return Base.extend({
		...
		initializeNestedViews: function initializeNestedViews() {
			...
			_(nesteds).each(function (context, index) {
				context = _this._normalizeNestedContext(context, index);
				_this._createNestedContext(context);
			});
		...
		_normalizeNestedContext: function _normalizeNestedContext(context, index) {
			var _this2 = this;
			...
			var name = _.isString(index) ? index : _.isString(context.name) ? context.name : _.uniqueId('nested');

			if (context.name != name) context.name = name;

			if (!_.isFunction(context.region) && _.isObject(context.region)) {
				var regionHash = context.region;
				context.region = function () {
					if (regionHash.updateDom) {
						if (_.isFunction(regionHash.updateDom)) {
							regionHash.updateDom.call(_this2, _this2.$el);
						} else if (regionHash.updateDom === true) {
							var selector = 'region-' + context.name;
							_this2.$el.append($('<div>').addClass(selector));
							regionHash.el = '.' + selector;
						}
					}
					var region = _this2.getRegion(regionHash.name || context.name);
					if (!region) {
						region = _this2.addRegion(regionHash.name || context.name, regionHash);
					}
					return region;
				};
			}

			if (!context.region && context.regionName) context.region = function () {
				return _this2.getRegion(context.regionName);
			};
		...
		showAllNestedViews: function showAllNestedViews() {
			...
			_(contexts).each(function (context) {
				return _this3.showNestedView(context.name);
			});
		},
		...
		_buildNestedView: function _buildNestedView(name) {
			var cfg = this.getNestedViewContext(name);
			if (!cfg) return;
			var View$$1 = cfg.View;
			var options = betterResult(cfg, 'options', { context: this, args: [this, this.model], default: {} });
		...
		getNestedViewRegion: function getNestedViewRegion(name) {
			var cfg = this.getNestedViewContext(name);
			if (!cfg && cfg.region) return;
			var region = cfg.region;
			if (_.isFunction(region)) region = region();
...
function getOption(context, key) {
	var opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
	...
	if (_.isFunction(context.getOption)) {
		...
		var fallback = _.isFunction(context.getProperty) ? context.getProperty : function (key) {
			return context[key];
		};
...
function getNotInitializedOption(key) {
	var opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

	var value = opts[key] != null ? opts[key] : this[key];
...
function getInputType(inputView) {
	...
	var valueType = getNotInitializedOption.call(inputView, 'valueType', opts);
	if (valueType == null) {
	...
	var type = getNotInitializedOption.call(inputView, 'inputType', opts);

	if (!type) {
		if (inputView._valueIsNumber) {
			type = getNotInitializedOption.call(inputView, 'inputNumberType', opts) || 'number';
...
function setInputAttributes(inputView) {
	...
	var attributes = getNotInitializedOption.call(inputView, 'attributes', opts);
	...
	_(restrictionKeys).each(function (key2, key) {
		var value = getNotInitializedOption.call(inputView, key, opts);
...
function getInputOption(context, key, ifNull) {
	var value = context.getOption(key, { args: [context.model, context] });
	if (value == null) {
		value = ifNull;
	}
	return value;
...
function keydown (eventContext) {
	...
	if (event.keyCode == 13 && getInputOption(context, 'doneOnEnter', true)) {
		prevent = true;
		stop = true;
	}
...
function blur (eventContext) {
	...
	if (getInputOption(context, 'doneOnBlur', true)) {
		context.triggerControlDone();
	}
}
...
function focus (eventContext) {
	...
	if (getInputOption(context, 'selectOnFocus', true)) {
		input.select();
	}
}
...
function setInputEvents(inputView) {
	...
	var passedEvents = getNotInitializedOption.call(inputView, 'events', opts);
