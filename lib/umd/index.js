...
}(this, (function (exports,backbone,Mn) { 'use strict';
...
function getOption(context, key, opts, also) {
	...
	var options = _.extend({ args: [context], context: context }, opts);
...
var ChildrenableMixin = (function (Base) {
	return Base.extend({
		...
		_prepareChildren: function _prepareChildren(items) {
			var _this2 = this;
			...
			if (filter != null && !_.isFunction(filter)) delete opts.filter;
		...
		_childFilter: function _childFilter(item, index) {
			...
			if (_.isFunction(filter) && !filter(item, index, opts)) return;
...
var RoutesMixin = {
	...
	registerAllRoutes: function registerAllRoutes() {
		...
		var pages = this.getAllChildren({ reverse: true });
		pages.push(this);
...
var index$9 = BasePage.extend({
	...
	getSiblings: function getSiblings() {
		...
		var pages = parent && parent.getChildren({ exclude: [this] }) || [];
		if (_.isFunction(opts.map)) return _(pages).chain().map(opts.map).filter(function (f) {
			return !!f;
		}).value();else return pages;
	},
	...
	_childFilter: function _childFilter(item, index) {
		...
		if (visible && (!item.visible || item.hidden)) return;
	}
});
