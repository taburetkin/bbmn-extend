...
var ChildrenableMixin = (function (Base) {
	return Base.extend({
		...
		getChildren: function getChildren() {
			...
			if (!reverse && !clone) return children;

			var result = [].slice.call(children);
			if (reverse) result.reverse();

			return result;
		},
		getAllChildren: function getAllChildren(opts) {
			var children = this.getChildren(opts);
			return _(children).chain().map(function (child) {
				return [child.getAllChildren(opts), child];
			}).flatten().value();
		},
...
var BaseRouter$2 = Router$1.extend({
	...
	startPage: function startPage(page) {
		...
		return this.beforePageStart().then(function () {
