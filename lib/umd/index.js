...
var ControlMixin = (function (Base) {
	return Base.extend({
		...
		tryValidateControl: function tryValidateControl(value) {
			var validate = getOption(this, 'validateControlValue', { force: false });
...
function setInputAttributes(inputView) {
	...
	var buildedAttributes = {
		value: inputView.value,
		type: getInputType(inputView, opts)
	};
	inputView.attributes = function () {
		return _.extend(buildedAttributes, attributes);
	};
}
...
var index$6 = (function (Base) {
	...
	return Mixin.extend({
		...
		getMaxLength: function getMaxLength() {
			var len = parseInt(this.getOption('maxLength'), 10);
