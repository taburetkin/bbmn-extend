(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('backbone'), require('backbone.marionette')) :
	typeof define === 'function' && define.amd ? define(['exports', 'backbone', 'backbone.marionette'], factory) :
	(factory((global.bbmn = {}),global.Backbone,global.Mn));
}(this, (function (exports,backbone,Mn$1) { 'use strict';
...
var BaseRouter = mix(backbone.Router).with(Mixin);
 ...
 */
	...
	getInitRoutes: function getInitRoutes() {
		var routes = void 0;
		if (!this.getOption('isMarionetteStyle')) routes = this.getOption('routes');else {
			var cntrl = this.getOption('controller') || {};
			var approutes = this.getOption('appRoutes') || {};
			routes = _(approutes).map(function (name, route) {
				return _.create({}, { route: route, name: name, callback: cntrl[name] });
			});
		}
		return routes;
	},
 ...
 */
	...
	route: function route(_route, name, callback) {
		...
		var context = this._normalizeRegisterRouteArguments(_route, name, callback, opts);
		...
		if (opts.isRouteChaining === true) return this;else return context;
	},
	...
	addRoutes: function addRoutes(routes) {
		...
		var normalized = _(routes).map(function (value, key) {
	...
	_normalizeRegisterRouteArguments: function _normalizeRegisterRouteArguments(route, name, callback) {
		var opts = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
		...
		if (!_.isFunction(context.callback) && opts.isRouterHoldsActions) context.callback = this[context.name];
	...
	_normalizeRegisterRouteContext: function _normalizeRegisterRouteContext(context) {
	...
	_normalizeRegisterRouteCallback: function _normalizeRegisterRouteCallback(context) {

		if (!_.isFunction(context.callback)) context.callback = function () {};
 ...
 */
	...
	_processCallback: function _processCallback(routeContext, fragment) {
		...
		var actionContext = this._createExecuteActionContext(routeContext, fragment, options);
		...
		var result = this.processCallback(actionContext, actionContext.routeType);
	...
	processCallback: function processCallback(actionContext, routeType) {

		var resultContext = {};
		var toPromise = this.getOption('callbacksAsPromises');
		var callback = function callback() {
			var result = actionContext.callback.apply(actionContext, arguments);
			if (toPromise) {
		...
		var args = this.getOption('classicMode') ? actionContext.rawArgs || [] : [actionContext];

		var event = this.execute(callback, args) !== false ? routeType : 'fail';
		this.triggerRouteEvents.apply(this, [actionContext, event, actionContext.name].concat(toConsumableArray(args)));

		var delegate = this.getOption('delegatePromiseErrorsTo');
		var catchErrors = this.getOption('catchPromiseErrors');
		resultContext.result.then(function (arg) {
			return arg;
		}, function (error) {
			if (delegate && _.isFunction(delegate.trigger)) {
				triggerMethodOn(delegate, 'route:error', actionContext, error);
			} else {
				if (!catchErrors) return Promise.reject(error);
			}
		});

		return resultContext.result;
	},
	_onReRouteLast: function _onReRouteLast() {
		if (!this.lastAttempt) return;
		var ac = this.lastAttempt.lastActionContext;
	...
	//   args:{},
	// }
	_createExecuteActionContext: function _createExecuteActionContext(context, fragment, options) {

		var rawArgs = this._extractParameters(context.route, fragment);
		var result = _.extend({}, context, { fragment: fragment, rawArgs: rawArgs }, options);
		...
		result.reroute = function () {
			var newcontext = _.extend({ routeType: 'reroute' }, result);
	...
	_prepareActionQueryString: function _prepareActionQueryString(args) {
		if (!_.isArray(args) || args.length == 0 || args.length == 1 && args[0] == null) return {};
		var rawQs = args.pop();
	...
	_prepareActionArguments: function _prepareActionArguments(fragment, rawRoute, args) {
