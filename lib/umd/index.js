...
var ChildrenableMixin = (function (Base) {
	return Base.extend({
		...
		_prepareChildren: function _prepareChildren(items) {
			...
			if (exclude != null && !_.isArray(exclude)) exclude = [exclude];

			if (filter != null && !_.isFunction(filter)) filter = null;

			if (!(exclude || filter || map)) return items;

			var result = [];
			_(items).each(function (item) {

				if (exclude && exclude.indexOf(item) >= 0) return;
