...
var Process = mix({
	...
	_getHookResultAsPromise: function _getHookResultAsPromise(hookName) {
		...
		var procHook = _.isFunction(this[procMethod]) && (_procMethod = this[procMethod]).call.apply(_procMethod, [this.context].concat(toConsumableArray(this.args))) || undefined;
	...
	triggerComplete: function triggerComplete() {
		...
		if (_.isFunction(this.onComplete)) (_onComplete = this.onComplete).call.apply(_onComplete, [this.context].concat(toConsumableArray(this.args)));
	...
	triggerError: function triggerError(errors) {
		...
		if (_.isFunction(this.onError)) (_onError = this.onError).call.apply(_onError, [this.context].concat(toConsumableArray(this.errors)));
...
var errorHandler = {
	handlers: {
		...
		'not:found': function notFound(fragment) {
	...
	_getHandleContext: function _getHandleContext(error, context) {
		...
		if (_.isFunction(this.getHandleContext)) return this.getHandleContext(error, context, args);
		...
		if (error instanceof Error) {
			...
			args.unshift(error);
			return { 'xhr': { context: context, args: args } };
		} else if (error instanceof $.Deferred().constructor) {
			args.unshift(error);
			return { 'jq:xhr': { context: context, args: args } };
		} else if (_.isString(error)) return defineProperty({}, error, { context: context, args: args });
	},
...
function findHandler(fragment, customTest) {
	...
	return _.filter(backbone.history.handlers || [], function (handler) {
...
function start(options) {
	...
	if (backbone.history.loadUrl !== historyLoadUrl) backbone.history.loadUrl = historyLoadUrl;
