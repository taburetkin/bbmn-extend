...
function getCallbackFunction(callback, executeResult, asPromise) {
	return function () {
		var result = callback && callback.apply(undefined, arguments);
...
function processCallback(router, actionContext, routeType) {
	...
	if (asPromise) {
		var delegate = router.getOption('delegatePromiseErrorsTo');
		var catchErrors = router.getOption('catchPromiseErrors');
		return executeResult.value.then(function (arg) {
			postProcessCallback({
				failed: arg === false,
				routeType: routeType, args: args, router: router, actionContext: actionContext
			});
			return arg;
		}, function (error) {
...
function postProcessCallback(_ref) {
	...
	var event = failed ? 'fail' : routeType;
	router.triggerRouteEvents.apply(router, [actionContext, event, actionContext.name].concat(toConsumableArray(args)));
