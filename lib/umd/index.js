...
var ProcessEngine = {
	process: function process(processContext) {
		...
		processContext.promise = new Promise(function (resolve, reject) {

			processContext.resolve = resolve;
			processContext.reject = reject;

			_this.triggerBegin(processContext);

			var endWithErros = function endWithErros() {
				for (var _len = arguments.length, errors = Array(_len), _key = 0; _key < _len; _key++) {
					errors[_key] = arguments[_key];
				}

				_this.triggerErrors(processContext, errors);
				_this.triggerEnd(processContext, true);
				return errors;
			};

			return _this.canNotRun(processContext).then(function () {
				_this.triggerBefore(processContext);

				var waitFor = _this.getPromises(processContext);
				if (waitFor != null && !_.isArray(waitFor)) waitFor = [waitFor];

				return Promise.all(waitFor).then(function () {
					return _this.triggerEnd(processContext);
				}, endWithErros);
			}, endWithErros);
		});
	...
	_triggerOnContext: function _triggerOnContext(processContext, eventName, args) {
	...
	_invokeOnContext: function _invokeOnContext(processContext, methodName) {
	...
	triggerBegin: function triggerBegin(processContext) {
	...
	triggerBefore: function triggerBefore(processContext) {
		this._triggerOnContext(processContext, 'before');
	},
	triggerEnd: function triggerEnd(processContext, fail) {
		if (!fail) this._triggerOnContext(processContext, '');
		this._triggerOnContext(processContext, 'end');
		if (fail) processContext.reject();else processContext.resolve();
		processContext.ended = true;
		var key = executingProcessFlagKey(processContext.name);
		delete processContext.context[key];
	},
	triggerErrors: function triggerErrors(processContext, errors) {
		var args = [errors, processContext.args];
	...
	canNotRun: function canNotRun(processContext) {
		var canNotRunPromise = this._invokeOnContext(processContext, 'can:not:' + processContext.name);
		...
		if (!_.isArray(canNotRunPromise)) canNotRunPromise = [canNotRunPromise];

		return Promise.all(canNotRunPromise);
	},
	getPromises: function getPromises(processContext) {
		var promises = this._invokeOnContext(processContext, 'get:' + processContext.name + ':promises');
...
function initializeProcess(context, name) {
	var opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
	...
	context[name] = function () {
		...
		var processContext = _.extend({
			name: name,
			context: context,
			args: args
		}, opts);
		return ProcessEngine.process(processContext);
	};
}
...
var index$10 = Object.freeze({
	...
	initProcess: initializeProcess
});



var index$11 = Object.freeze({
	processEngine: index$10
});
...
exports.utils = index$12;
exports.components = index$11;
