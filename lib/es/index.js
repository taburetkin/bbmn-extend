		...
		_getCssClassString: function _getCssClassString() {
			...
			var classes = _(modifiers).reduce(function (hash, modifier) {
				...
				if (_.isString(modifier)) {
					...
					cls = modifier.call(_this, _this.model, _this);
				}
				cls && (hash[cls] = true);
				return hash;
			}, {});

			return _.chain(classes).keys(classes).uniq().value().join(' ');
		},
...
_.extend(ViewManager.prototype, Events, borrow, collection, common, customs$1, models, render, EmptyViewMixin);
