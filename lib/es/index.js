...
import Mn, { triggerMethod } from 'backbone.marionette';

var MnObject = Mn.Object || Mn.MnObject;

var triggerMethod$1 = triggerMethod || Mn.triggerMethod;

var ctors = [Model, Collection, View, Router, MnObject];
...
function triggerControlEvent(control, event) {
	...
	var eventName = 'control:' + event;
	var namedEventName = name + ':' + event;
	...
	if (eventName != namedEventName) {
		method.call.apply(method, [control, namedEventName].concat(args));
	}

	control.proxyControlEventToParent.apply(control, [namedEventName].concat(args));
}
...
function mergeObject$$1(src, dst) {
...
var ControlMixin = (function (Base) {
	return Base.extend({
		initializeValues: function initializeValues() {
			var value = this.getOption('value', { deep: false, force: false });
		...
		setControlValue: function setControlValue(value) {
			...
			var same = compareObjects(this.value, newvalue);
			if (same) {
				return;
			}

			return this._validateControl(newvalue).then(function () {
				_this._previousValue = _this.value;
				_this.value = newvalue;
				trigger && _this.triggerControlChange();
				return Promise.resolve(_this.value);
			}, function (error) {});
		},
		isValid: function isValid() {
			return this._isInvalid === false;
		},
		_validateControl: function _validateControl(value) {
			var _this2 = this;
			...
			if (validateResult == null) {
				...
				if (_.isFunction(validateResult.then)) {
					return validateResult.then(function () {
						_this2._isInvalid = false;
					}, function (errors) {
						_this2._isInvalid = true;
						_this2.triggerControlInvalid(errors);
		...
		handleChildControlEvent: function handleChildControlEvent(eventName) {
  ...
  */
		...
		proxyControlEventToParent: function proxyControlEventToParent(eventName) {
			...
			(_handleChildControlEv = this.handleChildControlEvent).call.apply(_handleChildControlEv, [parent, eventName].concat(toConsumableArray(args)));
		}
	}, {
		ControlMixin: true
	});
...
var index$12 = (function (CollectionView) {
...
var index$13 = (function (CollectionView) {
	return CollectionView.extend({
		...
		_addChild: function _addChild(view) {
...
var index$14 = (function (CollectionView) {
	return CollectionView.extend({
...
var index$15 = Object.freeze({
	emptyFetch: index$12,
	improvedIndexes: index$13,
	nextCollectionView: index$14
});
...
var index$17 = BasePage.extend({
	...
	_childFilter: function _childFilter(item, index) {
		...
		if (!BasePage.prototype._childFilter.apply(this, arguments)) return;
...
export { index$16 as mixins, index$9 as utils, index$21 as components };
